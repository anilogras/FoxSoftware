...
{
    public partial class frmgrafikraporkozmetik : DevExpress.XtraEditors.XtraForm
    {
        public FoxSoftWareBusinessUOW _BusinesUOW;
        public frmgrafikraporkozmetik()
        {
            InitializeComponent();
        ...
        private void BtnYenile_Click(object sender, EventArgs e)
        {
            ...
            string sqlBuAy = "select sum(sdet.BirimFiyat)*sum(sdet.Miktar) as 'Ciro',sum(urun.Maliyet)*sum(sdet.Miktar) as 'Maliyet',(sum(sdet.BirimFiyat)*sum(sdet.Miktar))-(sum(Maliyet)*sum(sdet.Miktar)) as 'Kâr' from SatHrkDetays sdet left join SatHrkAnas sana on sana.Id = sdet.SatHrkAnaId left join Uruns urun on urun.Id = sdet.UrunId where urun.SatisTipi = 2 and sdet.Silinmis=0 and sana.Silinmis=0 and sana.Tarih between " + str_Ay_ilkGun + " and " + str_Ay_sonGun;
            string sqlAltiAy = "select sum(sdet.BirimFiyat)*sum(sdet.Miktar) as 'Ciro',sum(urun.Maliyet)*sum(sdet.Miktar) as 'Maliyet',(sum(sdet.BirimFiyat)*sum(sdet.Miktar))-(sum(Maliyet)*sum(sdet.Miktar)) as 'Kâr' from SatHrkDetays sdet left join SatHrkAnas sana on sana.Id = sdet.SatHrkAnaId left join Uruns urun on urun.Id = sdet.UrunId where urun.SatisTipi = 2 and sdet.Silinmis=0 and sana.Silinmis=0 and sana.Tarih between " + str_Alti_Ay_Once + " and " + str_Bugun;
            string sqlBuYil = "select sum(sdet.BirimFiyat)*sum(sdet.Miktar) as 'Ciro',sum(urun.Maliyet)*sum(sdet.Miktar) as 'Maliyet',(sum(sdet.BirimFiyat)*sum(sdet.Miktar))-(sum(Maliyet)*sum(sdet.Miktar)) as 'Kâr' from SatHrkDetays sdet left join SatHrkAnas sana on sana.Id = sdet.SatHrkAnaId left join Uruns urun on urun.Id = sdet.UrunId where urun.SatisTipi = 2 and sdet.Silinmis=0 and sana.Silinmis=0 and sana.Tarih >=" + str_BuYil;
        ...
        string sqlUrun= "select Top 10 urun.Adi as 'Argument', sum(shrkdet.miktar) as 'Value' from SatHrkDetays shrkdet left join SatHrkAnas shrkana on shrkana.Id=shrkdet.SatHrkAnaId left join Uruns urun on urun.Id = shrkdet.UrunId where urun.SatisTipi = 2 and shrkana.Silinmis= 0 and shrkdet.Silinmis=0 group by urun.Adi";
        string sqlSehir = "select top 10 il.Adi as 'Argument', sum(sdet.Miktar) as 'Value' from SatHrkDetays sdet left join SatHrkAnas sana on sana.Id=sdet.SatHrkAnaId left join Musteris mus on mus.Id=sana.MusteriId left join AdresBilgisis adres on adres.Id = mus.AdresId left join Ils il on il.Id =adres.IlId  left join Uruns urun on urun.Id = sdet.UrunId where urun.SatisTipi = 2 and sana.Silinmis=0 and sdet.Silinmis=0 group by il.Adi";
        private void BtnRaporla_Click(object sender, EventArgs e)
        {
            ...
            string sqlCinsiyetKosul = "select case when urun.Cinsiyet = 1 Then 'Erkek' when urun.Cinsiyet = 2 Then 'Kadın' when urun.Cinsiyet = 3 Then 'Unisex' end as 'Argument', sum(shrkdet.miktar) as 'Value' from SatHrkDetays shrkdet left join SatHrkAnas shrkana on shrkana.Id=shrkdet.SatHrkAnaId left join Uruns urun on urun.Id = shrkdet.UrunId where urun.SatisTipi = 2 and shrkana.Silinmis= 0 and shrkdet.Silinmis=0 and shrkana.Tarih between " + ilkTarih +" and " +sonTarih+" group by urun.Cinsiyet";
            string sqlUrunKosul = "select Top 10 urun.Adi as 'Argument', sum(shrkdet.miktar) as 'Value' from SatHrkDetays shrkdet left join SatHrkAnas shrkana on shrkana.Id=shrkdet.SatHrkAnaId left join Uruns urun on urun.Id = shrkdet.UrunId where urun.SatisTipi = 2 and shrkana.Silinmis= 0 and shrkdet.Silinmis=0 and shrkana.Tarih between " + ilkTarih + " and " + sonTarih + "  group by urun.Adi";
            PieChartLoad(sqlCinsiyetKosul,chartCinsiyet);
            PieChartLoad(sqlUrunKosul, chartSehir);
        }
